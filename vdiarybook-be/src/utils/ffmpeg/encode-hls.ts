...
const getResolution = async (filePath: string) => {
  ...
  console.log('>>> Checking resolution for file:', slash(filePath))

  const { stdout } = await $`ffprobe -v error -select_streams v:0 -show_entries stream=width,height -of csv=s=x:p=0 ${slash(filePath)}`


  console.log('Resolution stdout:', stdout)
  // if (!stdout.trim()) {
  //   throw new Error(`❌ Không đọc được độ phân giải video từ file: ${filePath}`)
  // }
