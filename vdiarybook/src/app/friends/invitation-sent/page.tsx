...
import { IFriendRequest, IFriendSentRequest } from "@/types/friend-request.type";
...
export default function FriendRequestsList() {
  const [requests, setRequests] = useState<IFriendSentRequest[]>([]);
  ...
  const fetchFriendRequests = async (pageNum: number) => {
    try {
      setLoading(true);
      const res = await friendApi.getFriendSentRequests(pageNum, 10);
      const requestList = res.data.data?.sentRequests || [];
